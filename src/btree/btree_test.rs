...
mod tests {
    use crate::{
        btree::{local_ownership::LocalOwnership, BTreeRespMeta},
        BTreeClient,
    };
    use obelisk::{
        FunctionalClient, HandlerKit, InstanceInfo, PersistentLog, ServerlessHandler,
        ServerlessStorage,
    };
    ...
    async fn run_simple_test() {
        ...
        let btree_actor = BTreeActor::new(kit).await;
    ...
    async fn leaf_split_merge_test() {
        ...
        reset_for_test().await;
        run_leaf_split_merge_test(1024).await;
        // Give time to write backs.
        tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    }

    async fn run_leaf_split_merge_test(insert_size_kb: usize) {
        // Make btree actor.
        let kit = make_test_actor_kit(true).await;
        let btree_actor = BTreeActor::new(kit).await;
        // Insert values.
        ...
        let min_num_blocks = insert_size_kb / LEAF_SIZE_KB;
        ...
        // Check that there is a sufficient number of blocks.
        tokio::time::sleep(std::time::Duration::from_millis(500)).await;
        let min_num_blocks = (insert_size_kb / LEAF_SIZE_KB) / 2 + 1;
    ...
    async fn setup_node(btree_client: &BTreeClient) {
    ...
    async fn run_simple_actor_test() {
        ...
        let _manager_mc = setup_node(&btree_client).await;
        // The empty key always maps to the empty value.
    ...
    //     // Cleanup all data.
    //     bc.cleanup().await;
    //     let manager_mc = bc.global_ownership.get_or_make_client(MANAGER_ID).await;
